								...
								<div class="left">
									<div class="coupon">
										...
										<select id="searchType" name="searchType">
									        ...
									        <option value="authorName">작가 이름</option>
					...
					<table class="table shopping-summery" style="box-shadow: 0px 0px 15px #0000001a;">
						<thead>
							<tr class="main-hading">
								...
								<th class="text-center" style="width: 5%;">재고</th>
								<th class="text-center" style="width: 10%;">등록날짜</th>
...
$(document).ready(function() {
    ...
    initializeForm();
    /* 첫 페이지부터 시작 */
    bookList(getUrlParameters().currentPage);

    /* 검색 폼 제출 이벤트 처리 */
    $('#searchForm').on('submit', function(e) {
        e.preventDefault(); // 기본 동작을 방지
        bookList(1); // 검색 시 첫 페이지부터 시작
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(response.totalCount > 0) {
                    $.each(response.bookList, function(index, list) {
                        bookListHtml += '<tr class="text-center">';
                        bookListHtml += '<td><span>' + ((response.totalCount - list.rnum) + 1) + '</span></td>';
                        bookListHtml += '<td><a href="/admin/book/detail/' + list.book_id + newUrl + '">' + list.book_name + '</a></td>';
                        bookListHtml += '<td><span>' + list.author_name + '</span></td>';
                        bookListHtml += '<td><span>' + list.category_name + '</span></td>';
                        bookListHtml += '<td><span>' + list.book_stock + '</span></td>';
                        bookListHtml += '<td><span>' + list.reg_date + '</span></td>';
                        bookListHtml += '<td><a href="#" data-delete="' + list.book_id + '"><i class="ti-trash remove-icon"></i></a></td>';
                        bookListHtml += '</tr>';
                    });
                } else {
                    bookListHtml += '<tr class="text-center">';    
                    bookListHtml += '<td colspan="7"><span>데이터가 존재하지 않습니다.</span></td>';
                    bookListHtml += '</tr>';
                }
	...
	/* 상품 삭제 */
  	$('#bookList').on('click', '[data-delete]', function(e) {	// 동적으로 생성된 요소 data-delete (AJAX로 추가된 항목들 등)에도 이벤트 핸들러를 연결
    	e.preventDefault();
        ...
        if (confirm('정말로 삭제하시겠습니까?')) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    bookList(getUrlParameters().currentPage); // 삭제 시점의 최신 파라미터 가져오기
                },
                error: function(xhr, status, error) {
